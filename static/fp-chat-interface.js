...
class ModernChatInterface {
  ...
  init() {
    ...
    this.selectedMode = this.options.mode || 'generate'; // Default generate_mode
  ...
  createNewChat(existingInputElementId = null, site = null) {
    ...
    // Show logo for new chat
    const logoTitle = document.querySelector('.logo-title');
    if (logoTitle) {
      logoTitle.classList.remove('hidden');
    }
    
    // Show headings for new chat
    const justAskBuckyHeading = document.querySelector('.just-askbucky-heading');
    const menuHeading = document.querySelector('.menu-heading');
    if (justAskBuckyHeading) {
      justAskBuckyHeading.classList.remove('hidden');
    }
    if (menuHeading) {
  ...
  sendMessage(messageText = null) {
    ...
    if (this.elements.welcomeMessage && !this.elements.welcomeMessage.classList.contains('hidden')) {
      this.elements.welcomeMessage.classList.add('hidden');
    }
    
    // Hide logo when user sends first message
    const logoTitle = document.querySelector('.logo-title');
    if (logoTitle && !logoTitle.classList.contains('hidden')) {
      logoTitle.classList.add('hidden');
    }
    
    // Hide headings when user sends first message
    const justAskBuckyHeading = document.querySelector('.just-askbucky-heading');
    const menuHeading = document.querySelector('.menu-heading');
    if (justAskBuckyHeading && !justAskBuckyHeading.classList.contains('hidden')) {
      justAskBuckyHeading.classList.add('hidden');
    }
    if (menuHeading && !menuHeading.classList.contains('hidden')) {
      menuHeading.classList.add('hidden');
    }
  ...
  getStreamingResponse(query) {
    ...
    const params = new URLSearchParams({
      ...
      generate_mode: this.selectedMode || 'generate',
      display_mode: 'full',
      site: this.selectedSite || 'all'
    });
