...
class GenerateAnswer(NLWebHandler):
    ...
    async def synthesizeAnswer(self): 
        ...
        try:
            ...
            if not response:
                logger.error("No response received from synthesis prompt")
                message = {
                    "message_type": "nlws", 
            ...
            if not isinstance(response, dict) or "answer" not in response:
                logger.error(f"Invalid response structure from synthesis prompt: {response}")
                message = {
                    "message_type": "nlws", 
                    "answer": "I encountered an error while generating your answer. Please try again.", 
                    "items": []
