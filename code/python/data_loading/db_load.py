...
async def detect_file_type(file_path: str) -> Tuple[str, bool]:
    ...
    """
     # ── Force JSON-LD if the filename ends in .jsonld ─────────────────────────
    if file_path.lower().endswith(".jsonld"):
        return "json-ld", False
    # -------------------------------------------------------------------------
        ...
        with open(file_path, 'r', encoding='utf-8', errors='replace') as csv_file:
            ...
            for index, row in enumerate(csv_reader):
                try:
                    ...
                    # Create document
                    document = {
                        "id": str(hash(f"{site}:{url}") % (2**63)),  # Create a stable ID from the site and URL
...
async def loadJsonToDB(file_path: str, site: str, batch_size: int = 100, delete_existing: bool = False, force_recompute: bool = False, database: str = None):
    ...
    try:
        ...
        if file_type == 'csv':
            ...
            # --- JSON-LD handler ------------------------------------------------
            print("+++ ENTERED JSON-LD BRANCH +++")

            # 1) Load the JSON-LD file
            with open(resolved_path, "r", encoding="utf-8") as f:
                data = json.load(f)

            # 2) Helper: yield every MenuItem object in a Menu
            def each_menu_item(menu):
                for section in menu.get("hasMenuSection", []):
                    for item in section.get("hasMenuItem", []):
                        yield item

            # 3) Collect Menu objects (file may hold one or a list)
            if isinstance(data, dict) and data.get("@type") == "Menu":
                menus = [data]
            elif isinstance(data, list):
                menus = [m for m in data if isinstance(m, dict) and m.get("@type") == "Menu"]
            else:
                menus = []

            all_documents = []        # ← this list is consumed later by the
                                      #    existing embed/upload logic
            for menu in menus:
                # Extract menu-level context information
                menu_date = menu.get("datePublished", "")
                menu_hall = menu.get("hall", "")
                menu_meal = menu.get("meal", "")
                
                for section in menu.get("hasMenuSection", []):
                    section_name = section.get("name", "")
                    
                    for mi in section.get("hasMenuItem", []):
                        # Add menu context to each menu item
                        mi_with_context = mi.copy()
                        mi_with_context["menuDatePublished"] = menu_date
                        mi_with_context["menuHall"] = menu_hall
                        mi_with_context["menuMeal"] = menu_meal
                        mi_with_context["menuSectionName"] = section_name
                        
                        url = mi.get("url") or f"menuitem:{mi.get('name','no-name')}"
                        doc_json = json.dumps(mi_with_context, ensure_ascii=False).replace("\n", " ")
                        doc = {
                            "id": str(hash(f"{site}:{url}") % (2**63)),
                            "schema_json": doc_json,
                            "url": url,
                            "name": mi.get("name", "Menu item"),
                            "site": site
                        }
                        all_documents.append(doc)
             # add these two lines ↓↓↓
            print(f"DEBUG built {len(all_documents)} docs for {resolved_path}")
            for d in all_documents[:3]:
                print("DEBUG sample doc", d["name"])   # shows first 3 names
            # -------------------------------------------------------------------
        else:
